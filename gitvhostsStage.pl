...
use strict;
use warnings;
open(my $file ,"<", "/tmp/stagegit.endirs") or die "nemoga da otvorq faila $!";
my @filec = <$file>;

my $dir="/home/git/public_html/stage";
my @tmpArr;
...
#			print "###########  ${filec[$i]}# == ${tmpArr[$j]}#\n";
		...
		{
			print "triem ${filec[$i]} \n";
			open(my $vfile,">/etc/apache2/sites-enabled/stage.git") or die $!;
			for(my $l=0;$l<=$#tmpArr;$l++)
			{
                        ${tmpArr[$l]} =~ s/\n//;
                        print $vfile "<VirtualHost *:80>\n";
                                print $vfile "        ServerName      ${tmpArr[$l]}.stage.vjsoft.net\n";
                        
			print $vfile "        ServerAdmin     hosting\@vjsoft.net\n";
			print $vfile "        DocumentRoot    /home/git/public_html/stage/${tmpArr[$l]}\n";}
...
my 	$diff=$#tmpArr-$#filec;
	...
	for(my $i=0;$i<=$#tmpArr;$i++)
	{
		...
		{       
			...
			open(my $vfile,">>/etc/apache2/sites-enabled/stage.git") or die $!;
			print $vfile "<VirtualHost *:80>\n";
				print $vfile "        ServerName      ${tmpArr[$i]}.stage.vjsoft.net\n";	
			
			print $vfile "        ServerAdmin     hosting\@vjsoft.net\n";
                        print $vfile "        DocumentRoot    /home/git/public_html/stage/${tmpArr[$i]}\n";
